...
import { Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Select, MenuItem, InputLabel } from "@mui/material"
    ...
    <SidebarProvider>
      ...
      <div className="flex">
        <div className="sidebar-container">
          <HRdashboardSidebar />
        </div>        <div className="salary-page-content w-full mx-auto my-10 flex flex-col gap-5 px-4 bg-white dark:bg-neutral-900">
          <div className="salary-heading flex justify-between items-center md:pe-5">
            <h1 className="min-[250px]:text-xl md:text-4xl font-bold text-gray-900 dark:text-neutral-100">Salary Management</h1>
          ...
          <div className="salary-table bg-white dark:bg-neutral-800 p-5 rounded-lg shadow border border-gray-200 dark:border-neutral-700">
          ...
          <Dialog.Root open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            <Dialog.Portal>              <Dialog.Overlay className="fixed inset-0 bg-black/50 dark:bg-black/70" />
              <Dialog.Content className="fixed top-[50%] left-[50%] max-h-[90vh] w-[95vw] max-w-[550px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-white dark:bg-neutral-800 p-6 shadow-lg overflow-y-auto border border-gray-200 dark:border-neutral-700">
                <Dialog.Title className="text-xl font-bold mb-2 text-gray-900 dark:text-neutral-100">Create New Salary</Dialog.Title>
                <Dialog.Description className="text-gray-600 dark:text-neutral-400 mb-4">
                  Enter the details for the new salary record.
                </Dialog.Description>
                
                <form onSubmit={handleSubmitCreate} className="grid gap-3 py-2">
              
                <div className="employee-selection mb-3">                  <label className="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-1">
                    Employee*
                  </label>
                  <div className="employee-buttons grid grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 border border-gray-300 dark:border-neutral-600 rounded bg-white dark:bg-neutral-700">
                    {employeesState?.data ? (
                      employeesState.data.map((employee) => (
                        <button
                          key={employee._id}
                          type="button"                          className={`p-2 text-left text-sm rounded transition-colors ${
                            formData.employeeID === employee._id
                              ? "bg-blue-500 text-white"
                              : "bg-gray-100 dark:bg-neutral-600 hover:bg-gray-200 dark:hover:bg-neutral-500 text-gray-900 dark:text-neutral-100"
                          }`}
                          onClick={() => {
                            setFormData({
                              ...formData,
                      ...
                      <div className="p-2 text-gray-500 dark:text-neutral-400">Loading employees...</div>
                    )}
                  </div>
                  {formData.employeeID ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    />                    <div className="currency-selection">
                      <InputLabel id="currency-select-label" className="text-sm mb-1 text-gray-700 dark:text-neutral-300">Currency</InputLabel>
                      <Select
                ...
                <Dialog.Close asChild>
                  <button
                    className="absolute top-4 right-4 inline-flex h-6 w-6 items-center justify-center rounded-full text-gray-500 dark:text-neutral-400 hover:text-gray-700 dark:hover:text-neutral-200"
                    aria-label="Close"
                  >
                    X
                  </button>
          ...
          <Dialog.Root open={isUpdateDialogOpen} onOpenChange={setIsUpdateDialogOpen}>
            <Dialog.Portal>              <Dialog.Overlay className="fixed inset-0 bg-black/50 dark:bg-black/70" />
              <Dialog.Content className="fixed top-[50%] left-[50%] max-h-[90vh] w-[95vw] max-w-[550px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-white dark:bg-neutral-800 p-6 shadow-lg overflow-y-auto border border-gray-200 dark:border-neutral-700">
                <Dialog.Title className="text-xl font-bold mb-2 text-gray-900 dark:text-neutral-100">Update Salary</Dialog.Title>
                <Dialog.Description className="text-gray-600 dark:text-neutral-400 mb-4">
                  Update the details for this salary record.
                </Dialog.Description>
                
                <form onSubmit={handleSubmitUpdate} className="grid gap-3 py-2">                  <div className="employee-selection mb-3">
                    <label className="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-1">
                      Employee* (Selected)
                    </label>
                    <div className="p-2 bg-gray-100 dark:bg-neutral-700 border border-gray-300 dark:border-neutral-600 rounded">
                      {employeesState?.data ? (
                        (() => {
                          const selectedEmployee = employeesState.data.find(
                            ...
                            <div className="text-gray-500 dark:text-neutral-400">Employee not found</div>
                          );
                        })()
                      ) : (
                        <div className="text-gray-500 dark:text-neutral-400">Loading employee details...</div>
                      )}
                    </div>
                  </div>                  <div className="status-selection mb-3">
                    <label className="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-1">
                      Status*
                    </label>
